...
module.exports = exports = function(opt_handler, opt_suffix, opt_forwardPathName) {
    ...
    return function(context) {
        ...
        if ( opt_forwardPathName ) {
            targetPathName = opt_forwardPathName(context);

            if (targetPathName) {
                targetURL = request.headers.host
                    + targetPathName + (request.search || '');
                edp.log.info(
                ...
                targetPathName = request.pathname;
            }
        }
        ...
        child.stderr
            .on( 'end', function () {
                edp.log.info('php error end [' + [].slice.call(arguments) + ']');
            })
            .on( 'data', function () {
                edp.log.info('php error data [' + [].slice.call(arguments) + ']');
            });
